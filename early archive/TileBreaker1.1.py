...
game_font = pg.font.SysFont('NewTimesRoman', 30)
    ...
    for x in range(0, ARRAY_WIDTH):
        for y in range(0, ARRAY_HEIGHT):
            ...
            tile_surface = game_font.render('{:02d}'.format(total[y][x]), False,
                                            (255, 255, 255))
            pg.draw.circle(screen, DARK_GRAY, (x * GRID_SIZE + GAMEBOARD_INSET, y * GRID_SIZE + GAMEBOARD_INSET), 30, 2)
            screen.blit(tile_surface, (x * GRID_SIZE + GAMEBOARD_INSET - 15, y * GRID_SIZE + GAMEBOARD_INSET - 15))
