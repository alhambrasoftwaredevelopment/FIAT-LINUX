...
pg.init()
pg.font.init()
title_font = pg.font.SysFont('Tahoma', int(150 / ARRAY_WIDTH))
game_font = pg.font.SysFont('NewTimesRoman', int(150 / ARRAY_WIDTH))
...
def draw_elements():
    pg.draw.rect(screen, DARK_GREEN, (OFFSET, OFFSET, GAMEBOARD_WIDTH + OFFSET, HEIGHT - (2 * OFFSET)), 1)
    pg.draw.line(screen, DARK_GREEN, (NEW_X + OFFSET_II, GAMEBOARD_INSET), (NEW_X + OFFSET_II, HEIGHT - 100), 1)
    pg.draw.line(screen, DARK_GREEN, (GAMEBOARD_INSET + 15, NEW_Y + OFFSET_III), (WIDTH - 365, NEW_Y + OFFSET_III), 1)
    ...
    for x in range(0, ARRAY_WIDTH):
        for y in range(0, ARRAY_HEIGHT):
            ...
            tile_surface = game_font.render('{:02d}'.format(total[y][x]), False,
                                            (255, 255, 255))
            pg.draw.circle(screen, DARK_GRAY, (x * GRID_SIZE + OFFSET_II, y * GRID_SIZE + OFFSET_III), 25, 1)
            screen.blit(tile_surface, (x * GRID_SIZE + OFFSET_II - 10, y * GRID_SIZE + OFFSET_III - 10))
    pg.draw.circle(screen, GREEN, (NEW_X + GAMEBOARD_INSET_II, NEW_Y + GAMEBOARD_INSET), 28, 1)
    score_write = game_font.render('MOVE COUNT: {}'.format(move_count), False, (255, 255, 255))
    screen.blit(score_write, (WIDTH - 250, HEIGHT / 4))
    high_score_write = game_font.render('RECORD SCORE: {}'.format(best_score), False, (255, 255, 255))
    screen.blit(high_score_write, (WIDTH - 250, HEIGHT - HEIGHT / 6))
    title = title_font.render('TILE BREAKER', False, (255, 255, 255))
    screen.blit(title, (WIDTH - 250, HEIGHT / 30))
    completion_rate = game_font.render('COMPLETION RATE:', False, (255, 255, 255))
    screen.blit(completion_rate, (WIDTH - 250, HEIGHT / 3))
    time_counter = game_font.render('TIME:{:02d} {:02d} secs'.format(round(counting_minutes), round(counting_seconds)),
                                    False, (255, 255, 255))
    screen.blit(time_counter, (WIDTH - 250, HEIGHT - HEIGHT / 4))
...
while running:
    ...
    if rotate_clock or rotate_counter == True:
        pg.draw.circle(screen, GREEN, (NEW_X + GAMEBOARD_INSET_II, NEW_Y + GAMEBOARD_INSET), 100, 25)
    if pull_left == True:
        pg.draw.line(screen, GREEN, (50, NEW_Y + GAMEBOARD_INSET),
                     (NEW_X + GAMEBOARD_INSET_II, NEW_Y + GAMEBOARD_INSET), 25)
    if pull_right == True:
        pg.draw.line(screen, GREEN, (NEW_X + GAMEBOARD_INSET + 15, NEW_Y + GAMEBOARD_INSET),
                     (WIDTH - 300, NEW_Y + GAMEBOARD_INSET), 25)
    if pull_up == True:
        pg.draw.line(screen, GREEN, (NEW_X + GAMEBOARD_INSET + 20, 50),
                     (NEW_X + GAMEBOARD_INSET + 20, NEW_Y + GAMEBOARD_INSET), 25)
    if pull_down == True:
        pg.draw.line(screen, GREEN, (NEW_X + GAMEBOARD_INSET + 20, NEW_Y + GAMEBOARD_INSET),
